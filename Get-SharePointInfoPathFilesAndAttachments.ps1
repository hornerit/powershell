...
OPTIONAL By default, the date filters are based on when the infopath file was created. Use this switch to use LastModified attribute of the file instead of CreatedDate.
...
if(!($DownloadOnly)){
	...
	Write-Progress -id 1 -activity "Step 2 of 2: Extracting Attachments" -status "Working on $loopCounter of $loopTotal (Updates every $interval files processed)" -percentComplete 0
	foreach($file in $myFiles){
		$fileErrorCounter = 0
		$loopCounter++
		if(($loopCounter % $interval) -eq 0){
			Write-Progress -id 1 -activity "Step 2 of 2: Extracting Attachments" -status "Working on $loopCounter of $loopTotal (Updates every $interval files processed)" -percentComplete ($loopCounter/$loopTotal*100)
		}
